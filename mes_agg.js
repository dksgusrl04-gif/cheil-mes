...
(function (root) {
  ...
  /* 마모형 가중치 — 설계 배분 50 : 30 : 10.
     조건급변은 0 으로 내렸다 — 급변 순간에 진동이 오히려 42.6% 떨어져서
     «충격을 받는다» 는 전제가 데이터로 안 받쳐진다. 지우지 않고 0 으로 두어,
     진동 첨도가 수집되면 되살릴 수 있게 했다.
     알림(Alarm)은 전 구간 고정값이라 계산에 안 넣는다.
     총량은 바꾸기 전과 같다. 실현 기여도는 55.56 / 33.33 / 11.11 %.
     mes_rebuild.py 의 UNIT 과 한 글자도 달라선 안 된다. */
  const UNIT = {
    machining: 0.0000578462, overload: 0.0084889543,
    shock: 0.0026756135, cond_change: 0.0000000000,
  };
  /* ── 파손형은 다른 가중치를 쓴다 ─────────────────────────────
  ...
  function finish(segs, meta) {
    ...
    return {
      ...
      units: { wear: UNIT, breakage: UNIT_BREAK,
        ...
        wear_mix: { machining: 50, overload: 30, shock: 10, cond_change: 0, alarm: 10 },
        wear_note: '설계 배분 실가공 50 · 과부하 30 · 충격 10 · 알림 10. '
          + '알림은 Alarm 컬럼이 전 구간 고정값이라 «미수집» 이고, 조건급변은 '
          + '근거가 약해 0 으로 두었습니다. 그래서 실제 기여도는 '
          + '실가공 55.56 · 과부하 33.33 · 충격 11.11 % 입니다.',
        missing: { 알림: 'Alarm·AlarmStatus·AlarmMsg 가 전 구간 고정값, Emergency 는 절삭 구간에 0건',
          조건급변: '급변 순간 진동이 오히려 42.6% 감소 — 전제가 데이터로 안 받쳐져 0 으로 둠' } },
      weights: { 마모형: '실가공 50 · 과부하 30 · 충격 10 · 알림 10(미수집)',
